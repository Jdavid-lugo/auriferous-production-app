        ...
        <div class="row">
            <div class="col-xl-12 order-xl-1">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form method="post" action="{{ route('molinos.store') }}" autocomplete="off">
                            @csrf
                            ...
                            <div class="pl-lg-4">
                                ...
                                <div class="form-group{{ $errors->has('rif') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" name="rif" id="input-rif" class="form-control form-control-alternative{{ $errors->has('rif') ? ' is-invalid' : '' }}" placeholder="{{ __('rif') }}" value="{{ old('rif') }}" required>
                                    @include('alerts.feedback', ['field' => 'rif'])
                                </div>
                                <div class="form-group{{ $errors->has('tlf') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-tlf">{{ __('tlf') }}</label>
                                    <input type="text" name="tlf" id="input-tlf" class="form-control form-control-alternative{{ $errors->has('tlf') ? ' is-invalid' : '' }}" placeholder="{{ __('Telefono') }}" value="" required>
                                    @include('alerts.feedback', ['field' => 'tlf'])
                                </div>
                                <div class="form-group">
                                    <label class="form-control-label" for="input-sector">{{ __('Confirmar sector') }}</label>
                                    <input type="text" name="sector" id="input-sector" class="form-control form-control-alternative" placeholder="{{ __('Confirmar sector') }}" value="" required>
                                </div>
