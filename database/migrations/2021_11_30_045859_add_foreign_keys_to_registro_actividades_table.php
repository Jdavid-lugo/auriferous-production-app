...
class AddForeignKeysToRegistroActividadesTable extends Migration {
	...
	public function up()
	{
		Schema::table('registro_actividades', function(Blueprint $table)
		{
			...
			$table->foreign('accion_id', 'registro_actividades_accion_id_fkey')->references('id')->on('accion')->onUpdate('RESTRICT')->onDelete('RESTRICT');
		});
	}
	...
	public function down()
	{
		Schema::table('registro_actividades', function(Blueprint $table)
		{
			...
			$table->dropForeign('registro_actividades_accion_id_fkey');
		});
	}
